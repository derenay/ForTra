...
class HierarchicalFormationTransformer(nn.Module):
    def __init__(self, 
        ...
        # Bölümlendirmeyi 3 yerine 4 parçaya çıkarıyoruz: coord, class, relative coord ve angle.
        self.dim_coord = initial_dim // 4
        self.dim_class = initial_dim // 4
        self.dim_angle = initial_dim // 4   # Yeni: tank yönü bilgisi
        self.dim_rel = initial_dim - (self.dim_coord + self.dim_class + self.dim_angle)
        ...
        # Yeni: açı bilgisini embedding’e sokmak için
        # Açı bilgisi tek bir sayı (örneğin, normalize edilmiş açı) olarak verilecek.
        self.angle_fc = nn.Linear(1, self.dim_angle)
        # Toplam birleşik boyutu initial_dim’e indirgemek için ek bir projeksiyon
        self.fuse_proj = nn.Linear(initial_dim + self.dim_angle, initial_dim)
    ...
    def forward(self, coords, class_tokens, angles):
        """
        coords: (B, N, 2)  -- tank konumları
        class_tokens: (B, N) -- tank sınıfı
        angles: (B, N) or (B, N, 1) -- tank yön açıları (normalize edilmiş, örneğin 0-1 arası)
        """
        ...
        # Yeni: açı bilgisini işliyoruz
        if angles.dim() == 2:
            angles = angles.unsqueeze(-1)
        angle_emb = self.angle_fc(angles)
        
        # Orijinalde fused_emb, coord, class ve rel emb'leri ile oluşturuluyordu.
        # Şimdi açı embedding’ini de ekliyoruz.
        fused_emb = torch.cat([coord_emb, class_emb, rel_emb, angle_emb], dim=-1)
        # fuse_proj ile boyutu initial_dim’e indiriyoruz
        fused_emb = self.fuse_proj(fused_emb)
        
        x = self.pos_encoder(fused_emb)
        edge_bias = self.compute_edge_bias(coords)

        x = self.encoder_stage1(x, edge_bias)
        x = self.proj_stage2(x)
        x = self.encoder_stage2(x, edge_bias)
        x = self.proj_stage3(x)
        x = self.encoder_stage3(x, edge_bias)
