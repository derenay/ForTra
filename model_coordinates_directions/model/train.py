...
def main(config):
    ...
    if config['load_path']:
        ...
        try:
            load_model(path=config['load_path'], model=model, device=device)
...
if __name__ == "__main__":
    # --- Configuration (Önerilen Ayarlar) ---
    config = {
        # Data and Save Paths
        'data_file': 'dataset/data.json',
        'save_dir': 'trained_models',
        'run_name': f'hft_balanced_run_{time.strftime("%Y%m%d_%H%M%S")}', # Ayarları yansıtan isim

        # Model Hyperparameters
        'class_embed_dim': 32,         # Sınıf sayısı azsa yeterli.
        'direction_dim': 1,            
        'stage_dims': [256, 128],      # İlk aşamaya daha fazla kapasite, sonra daraltma. (Önceki: [128, 64])
        'num_heads': 8,                # Daha standart, kafa başına daha büyük boyut sağlar. (Önceki: 16)
        'num_layers': [6, 6],          # Toplam 12 katman, güçlü ama aşırı derin değil. (Önceki: [8, 8])
        'dropout_stages': [0.2, 0.1],  # Daha makul dropout oranları. (Önceki: [0.5, 0.2])
        'use_adapter': True,           # Adapter kullanımı devam.
        'adapter_dim': 32,             # Boyutlara göre biraz artırıldı, hala darboğaz. (Önceki: 16)
        'pos_type': 'learnable',
        'max_len': 50,               # Verisetindeki max uzunluğa göre gerekirse ayarlanabilir.
        'ffn_ratio': 4,                # Standart feed-forward oranı.

        # Training Hyperparameters
        'epochs': 20,                  # Biraz daha uzun eğitim denenebilir. (Önceki: 50)
        'batch_size': 32,              # Belleğe göre ayarlanabilir.
        'lr': 0.0001,                  # AdamW için makul başlangıç.
        'weight_decay': 0.01,          # AdamW için standart.
        'lr_step_size': 15,            # LR düşürme adımı biraz uzatıldı. (Önceki: 10)
        'lr_gamma': 0.5,               # LR düşürme oranı.
        'grad_clip_norm': 1.0,
