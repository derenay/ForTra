...
class HierarchicalFormationTransformer(nn.Module):
    ...
    def compute_directional_bias(self, directions: torch.Tensor) -> torch.Tensor:
        ...
        # self.direction_dim modelin __init__'inde tanımlanmış olmalı
        ...
        if self.direction_dim == 1:
            # <<< YENİ: 0-1 Aralığını 0-2pi Radyana Ölçeklendirme >>>
            directions_rad = directions * (2 * math.pi)

            # Radyan cinsinden açı farklarını hesapla: [B, S, 1] - [B, 1, S] -> [B, S, S]
            dir_diff = directions_rad.unsqueeze(2) - directions_rad.unsqueeze(1)

            # Farkın kosinüsünü kullan. Açısal döngüyü doğal olarak ele alır.
            # Hizalı = cos(0)=1, Zıt = cos(pi)=-1, Dik = cos(pi/2)=0
            directional_bias = torch.cos(dir_diff)

        elif self.direction_dim == 2:
            # 2D vektörler olduğunu varsay. Önce birim vektörlere normalize et.
            directions_norm = F.normalize(directions, p=2, dim=-1, eps=1e-6)
            # Çiftler arası kosinüs benzerliğini hesapla: (B, S, D) @ (B, D, S) -> (B, S, S)
            directional_bias = torch.bmm(directions_norm, directions_norm.transpose(1, 2))
        else:
            # Desteklenmeyen boyutlar için geri dönüş/hata
