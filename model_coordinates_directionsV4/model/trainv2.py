...
try:
    ...
    from modeldenemesi import HierarchicalFormationTransformer # model.py kullandığınızı varsayıyorum
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'run_name': f'hft_learnable_bias_run_{time.strftime("%Y%m%d_%H%M%S")}', 

        # ---------------------------------------------------
        #          Model Hyperparameters
        # ---------------------------------------------------
        'class_embed_dim': 32,         # Sınıf sayısı azsa yeterli.
        'direction_dim': 1,            # <<< VERİNİZE GÖRE AYARLAYIN (1: açı, 2: vektör)
        'coord_dim': 2,                # Koordinat boyutu (genellikle 2)
        'stage_dims': [256, 128],      # Önerilen dengeli yapı
        'num_heads': 8,                # Önerilen kafa sayısı
        'num_layers': [6, 6],          # Önerilen katman sayısı
        'dropout_stages': [0.3, 0.1],  # Önerilen dropout oranları
        'use_adapter': True,
        'adapter_dim': 32,
        'max_len': 50,                 # Max birim sayısı (bias limitleri için değil ama padding vb. için hala önemli olabilir)
        'ffn_ratio': 4,
        'num_spatial_bins': 16,        # Mekansal mesafe kovası sayısı (ayarlanabilir)
        'max_distance': 1.5,           # Dikkate alınacak max mesafe (0-1 arası koordinatlar için) (ayarlanabilir)
        'num_directional_bins': 16,    # Yönelimsel ilişki kovası sayısı (ayarlanabilir)
        # ---------------------------------------------------
